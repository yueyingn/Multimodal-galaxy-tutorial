...
__all__ = [
    ...
    "fm_xxlarge_24e_24d_swiglu_qknorm_nobias",
]
...
class FourM(nn.Module):
    ...
    def __init__(
        ...
        # Width (in bin units) of the Gaussian smear applied to the CE target
        # for ordinal modalities (those whose modality_info["is_ordinal"] is
        # True). 0 disables smoothing — falls back to standard one-hot CE.
        self.soft_ce_sigma = float(soft_ce_sigma)
    ...
    def cat_decoder_tensors(
        ...
        mod_dict = {
            mod: d for mod, d in random.sample(list(mod_dict.items()), len(mod_dict))
        }
        ...
        for mod, d in mod_dict.items():
            if self.modality_info[mod]["type"] in ["seq", "seq_emb", "seq_token"]:
                ...
                decoder_mask_all.append(
                    torch.logical_or(d["target_mask"][:, 1:], d["target_mask"][:, :-1])
                )
                ...
                mod_mask_all.append(
                    torch.full_like(
                        d["ids"][:, :-1],
                        self.modality_info[mod]["id"],
                        dtype=torch.int16,
                    )
                )
            else:
                # Important: For 2d / image modalities, the decoder input tokens are replaced by the mask token
                decoder_tokens_all.append(
                    torch.zeros_like(d["x"]) + self.mask_token
                )  # Replace x by mask token
                ...
                mod_mask_all.append(
                    torch.full_like(
                        d["ids"], self.modality_info[mod]["id"], dtype=torch.int16
                    )
                )
    ...
    def adapt_decoder_attention_mask(
        ...
        if self.decoder_causal_mask:
            ...
            causal_mask = torch.ones(
                (N, N), dtype=torch.bool, device=decoder_attention_mask.device
            ).triu(1)
        ...
        if self.decoder_sep_mask:
            ...
            sep_mask = repeat(mod_mask, "b n2 -> b n1 n2", n1=N) != repeat(
                mod_mask, "b n1 -> b n1 n2", n2=N
            )
    ...
    def _ordinal_ce(
        ...
        """Cross-entropy with a Gaussian-smoothed target distribution.

        Replaces the one-hot target at bin k with N(k, sigma^2) over the full
        codebook (bin units), normalised. This gives the model partial credit
        for "close" predictions and is the standard fix for treating an
        inherently ordinal target with categorical CE. Reduces to one-hot CE
        as sigma -> 0; reduces to uniform target as sigma -> infinity.

        Args:
            logits:  (N, V) raw logits over the codebook.
            targets: (N,)   integer ground-truth bin indices.
            sigma:   smoothing width in bin units (e.g. 1.0).

        Returns:
            Mean soft cross-entropy (scalar).
        """
        V = logits.size(-1)
        bins = torch.arange(V, device=logits.device, dtype=logits.dtype)
        diff = bins.unsqueeze(0) - targets.to(logits.dtype).unsqueeze(1)
        log_target = -0.5 * (diff / sigma) ** 2
        log_target = log_target - torch.logsumexp(log_target, dim=-1, keepdim=True)
        log_pred = F.log_softmax(logits, dim=-1)
        return -(log_target.exp() * log_pred).sum(dim=-1).mean()
    ...
    def _mod_ce(
        ...
        """Cross-entropy for a single modality. Uses ordinal Gaussian-smoothed
        CE if (a) self.soft_ce_sigma > 0 and (b) modality_info[mod]['is_ordinal']
        is True; otherwise standard categorical CE."""
        if self.soft_ce_sigma > 0 and self.modality_info[mod].get("is_ordinal", False):
            return self._ordinal_ce(logits, targets.long(), self.soft_ce_sigma)
    ...
    def forward_loss(
        ...
        if loss_type in ["mod", "modality"]:
            loss, mod_loss = self.forward_mod_loss(
                y, target_ids, decoder_mod_dict, decoder_mod_mask
            )
    ...
    def forward_mod_loss(
        ...
        for mod, d in decoder_mod_dict.items():
            ...
            if logits.numel() == 0:
                ...
                mod_loss[mod] = self._mod_ce(
                    mod, logits, target_ids[decoder_mod_mask == idx]
                )
    ...
    def forward_token_loss(
        ...
        loss = sum([mod_loss[mod] * mod_count[mod] for mod in mod_loss.keys()]) / sum(
            mod_count.values()
        )
    ...
    def forward(
        ...
        # Logits
        if return_logits:
            mod_logits = self.forward_logits(
                y, decoder_mod_dict, decoder_mod_mask, return_all_logits=True
            )
            return mod_logits

        # Loss
        loss, mod_loss = self.forward_loss(
            y, target_ids, decoder_mod_dict, decoder_mod_mask, loss_type
        )
    ...
    def freeze_params_except_specific_embeddings(self, frozen_embedding_domain):
        self.freeze_encoder_except_specific_embeddings(
            frozen_embedding_domain=frozen_embedding_domain
        )
        self.freeze_decoder_except_specific_embeddings(
            frozen_embedding_domain=frozen_embedding_domain
        )
...
class FM(FourM, PyTorchModelHubMixin):
    ...
    def __init__(self, config: dict):
        ...
        all_domains = sorted(
            list(set(config["domains_in"]) | set(config["domains_out"]))
        )
        ...
        for mod in config["domains_in"]:
            ...
            if info.get("encoder_embedding", None) is not None:
                # Original handling of images has been removed
